...
namespace KoggeStone {
    ...
    __device__ BitBoard ShiftUp(BitBoard b) { return  b << 8; }
    __device__ BitBoard ShiftDown(BitBoard b) { return  b >> 8; }
    __device__ BitBoard ShiftLeft(BitBoard b) { return (b << 1) & C64(0xfefefefefefefefe); }
    __device__ BitBoard ShiftRight(BitBoard b) { return (b >> 1) & C64(0x7f7f7f7f7f7f7f7f); }
    __device__ BitBoard ShiftUpLeft(BitBoard b) { return (b << 9) & C64(0xfefefefefefefefe); }
    __device__ BitBoard ShiftUpRight(BitBoard b) { return (b << 7) & C64(0x7f7f7f7f7f7f7f7f); }
    __device__ BitBoard ShiftDownLeft(BitBoard b) { return (b >> 7) & C64(0xfefefefefefefefe); }
    __device__ BitBoard ShiftDownRight(BitBoard b) { return (b >> 9) & C64(0x7f7f7f7f7f7f7f7f); }
    ...
    */
    __device__ BitBoard FillUpOccluded(BitBoard g, BitBoard p)
    {
        g |= p & (g << 8);
        p &= (p << 8);
        g |= p & (g << 16);
    ...
    __device__ BitBoard FillDownOccluded(BitBoard g, BitBoard p)
    {
        g |= p & (g >> 8);
        p &= (p >> 8);
        g |= p & (g >> 16);
    ...
    __device__ BitBoard FillLeftOccluded(BitBoard g, BitBoard p)
    {
    ...
    __device__ BitBoard FillRightOccluded(BitBoard g, BitBoard p)
    {
    ...
    __device__ BitBoard FillUpLeftOccluded(BitBoard g, BitBoard p)
    {
        p &= C64(0xfefefefefefefefe);
        g |= p & (g << 9);
        p &= (p << 9);
    ...
    __device__ BitBoard FillUpRightOccluded(BitBoard g, BitBoard p)
    {
        p &= C64(0x7f7f7f7f7f7f7f7f);
        g |= p & (g << 7);
        p &= (p << 7);
    ...
    __device__ BitBoard FillDownLeftOccluded(BitBoard g, BitBoard p)
    {
        p &= C64(0xfefefefefefefefe);
        g |= p & (g >> 7);
        p &= (p >> 7);
    ...
    __device__ BitBoard FillDownRightOccluded(BitBoard g, BitBoard p)
    {
        p &= C64(0x7f7f7f7f7f7f7f7f);
        g |= p & (g >> 9);
        p &= (p >> 9);
    ...
    */
    __device__ uint64_t direction_attacks(int square, int dir, uint64_t occ)
    {
    ...
    */
    __device__ uint64_t attacks_rook(int square, uint64_t occ)
    {
        uint64_t seed = 1ull << square;
        uint64_t free = ~occ;
        return ShiftRight(FillRightOccluded(seed, free))
    ...
    */
    __device__ BitBoard attacks_bishop(int square, uint64_t occ)
    {
        uint64_t seed = 1ull << square;
        uint64_t free = ~occ;
        return ShiftUpRight(FillUpRightOccluded(seed, free))
    ...
    __device__ uint64_t Queen(int square, uint64_t occ) {
