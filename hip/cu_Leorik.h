...
namespace Leorik
{
    ...
    __inline__ __device__ uint64_t MaskHigh(uint64_t bb) {
        return 0x7FFFFFFFFFFFFFFFUL >> countl_zero(bb | 1);
    }

    //identify the lowest set bit and set all bits below while zeroing the rest
    __inline__ __device__ uint64_t MaskLow(uint64_t bb)
    {
        return bb ^ (bb - 1);
    }
    ...
    __inline__ __device__ uint64_t VerticalShift(uint64_t bb, int ranks)
    {
        return ranks > 0 ? bb >> (ranks << 3) : bb << -(ranks << 3);
    }

    __inline__ __device__ uint64_t GenLine(uint64_t bbLine, uint64_t bbBlocker, uint64_t bbBelow)
    {
    ...
    __inline__ __device__ uint64_t GenLines(uint64_t bbLineA, uint64_t bbLineB, uint64_t bbBlocker, uint64_t bbBelow) {
    ...
    __inline__ __device__ uint64_t Rook(int square, uint64_t occupation)
    {
    ...
    __inline__ __device__ uint64_t Bishop(int square, uint64_t occupation)
    {
        uint64_t bbPiece = 1ULL << square;
        uint64_t bbBlocker = occupation & ~bbPiece;
        //mask the bits below bbPiece
    ...
    static __device__ uint64_t Queen(int sq, uint64_t occupation) {
