...
struct Cuda_Chessprocessor
{
    ...
    {
        gpuErrchk(hipMalloc(&attacks, threads * sizeof(uint64_t)));
        gpuErrchk(hipMemset(attacks, 0, threads * sizeof(uint64_t)));
    }
...
void TestChessprocessor(int blocks, int threadsperblock) {
    ...
    //Optimize bitrotation algorithm (horizontal ray). New world record: 142 Billion Lookups/S for queens. RTX 3080 28.04.2022
    {
        ...
        for (int i = 0; i < 16; i++) 
        {
            hipLaunchKernelGGL(HIP_KERNEL_NAME(cu_GetQueenAttacks<mode>), dim3(blocks), dim3(threadsperblock), 0, streams[i % streamcount], p);
            cudaVerifyLaunch();
        }
        for (int i = 0; i < streamcount; i++)
        {
...
void SetupDevice()
{
    ...
    gpuErrchk(hipDeviceSetSharedMemConfig(hipSharedMemConfig::hipSharedMemBankSizeEightByte));
